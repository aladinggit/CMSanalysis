...
//TH1F *j1 = new TH1F("j1","subleading lepton phi",20,-4.,4.);
TH1F *j1 = new TH1F("j1","subleading lepton phi",22,-3.3,3.3);
T1->Draw("subleading_phi>>j1","totalweight");  
  j1->SetLineColor(kBlack);
  j1->SetLineWidth(2);
  j1->SetStats(0);
TH1F *j2 = new TH1F("j2","subleading lepton phi",22,-3.3,3.3);
T2->Draw("subleading_phi>>j2","startweight"); 
  j2->SetLineColor(kBlue);
  j2->SetLineWidth(2);
  j2->SetStats(0); 
  j2->GetYaxis()->SetLabelSize(0.020);
TH1F *j3 = new TH1F("j3","subleading lepton phi",22,-3.3,3.3);
T3->Draw("subleading_phi>>j3","totalweight");
  j3->SetLineColor(kGreen);
  j3->SetLineWidth(2);
  j3->SetStats(0);
TH1F *j4 = new TH1F("j4","subleading lepton phi",22,-3.3,3.3);
T4->Draw("subleading_phi>>j4","startweight");
...
//  h1->GetYaxis()->SetRangeUser(700, 1450);
  ...
  h3->SetMaximum(1.8);
  h3->SetTitle("");
  h3->GetXaxis()->SetTitle("Sub leading Lepton phi ");
  ...
  tleg3->Draw("sames");
//gPad->Update();
c1->SaveAs("Lepton_subleading_phi.png");
c1->Write();
