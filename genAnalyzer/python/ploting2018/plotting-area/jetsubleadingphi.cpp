...
TH1F *j1 = new TH1F("j1","SubLeading Jet phi",22,-3.3,3.3);
T1->Draw("subleadingjet_phi>>j1","totalweight*1");  
  j1->SetLineColor(kBlack);
  j1->SetLineWidth(2);
  j1->SetStats(0);

TH1F *j2 = new TH1F("j2","SubLeading Jet phi",22,-3.3,3.3);
T2->Draw("subleadingjet_phi>>j2","startweight");                                                                                                
  j2->SetLineColor(kBlue);
  j2->SetLineWidth(2);
  j2->SetStats(0); 
  j2->GetYaxis()->SetLabelSize(0.020);
TH1F *j3 = new TH1F("j3","SubLeading Jet phi",22,-3.3,3.3);
T3->Draw("subleadingjet_phi>>j3","totalweight*1");
  j3->SetLineColor(kGreen);
  j3->SetLineWidth(2);
  j3->SetStats(0);

TH1F *j4 = new TH1F("j4","SubLeading Jet phi",22,-3.3,3.3);
T4->Draw("subleadingjet_phi>>j4","startweight");
...
TPad *pad2 = new TPad("pad2","pad2",0,0,1,0.2);
  ...
  h1->SetMaximum(1.8);
  h1->SetTitle("");
  h1->GetXaxis()->SetTitle("SubLeading Jet phi");
...
c1->SaveAs("SubJet_leading_phi.png");
c1->Write();
