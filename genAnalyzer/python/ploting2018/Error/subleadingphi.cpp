...
//  j1->GetYaxis()->SetRangeUser(700,900);
  j1->SetLineColor(kBlack);
  j1->SetLineWidth(2);
  j1->SetStats(0);
//TH1F *j2 = new TH1F("j2","subleading lepton phi",20,-4,4.);
TH1F *j2 = new TH1F("j2","subleading lepton phi",22,-3.3,3.3);
T2->Draw("subleading_phi>>j2","startweight"); 
//  j2->GetYaxis()->SetRangeUser(700, 1500);                                                                                               
...
auto gr = new TGraphErrors(h1 ->GetNbinsX(),X ,Y , 0, E);
   gr->SetTitle("Subleading Lepton Phi ratio");
   gr->SetMarkerColor(4);
   gr->SetMarkerStyle(21);
   gr->Draw("ALP");

  tleg1->AddEntry(gr,"Subleading Lepton Phi ratio","l");
