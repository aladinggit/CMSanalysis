...
#include <iostream>
#include <cstdlib>
TFile f1("/afs/cern.ch/user/h/hongyi/CMSSW_10_3_0/src/GenAnalyzer/CMSanalysis/genAnalyzer/python/ploting2018/Tgraph/plots.root");
TH1D *T2 = (TH1D*)f1.Get("j1");
void ana::Loop()
{
...
//        TFile ofile("ww.root", "recreate");
        TFile *ofile = new TFile("/eos/user/h/hongyi/ww_data/Powheg/powheg2016-r.root", "recreate");
...
*/
	...
	ww ->Branch("weights1005", &weights1005);
        ww ->Branch("weightsrevised", &weightsrevised);
...
int n = T2->GetNbinsX();
double X[n], Y[n], E[n];
for(int i=0;i< T2->GetNbinsX(); i++ ){
    X[i] = T2->GetBinCenter(i);
    Y[i] = T2->GetBinContent(i);
    E[i] = T2 ->GetBinError(i);
}
...
//                                nmuon_e->Fill(lmuvector.E(), weights);
   ...
   int n_jet = 0;
	...
	if (num_jet <= 6){
		weightsrevised =  Y[num_jet];
}
	else weightsrevised = 1;
